...
import { MYCEL_HOME_ENV } from '#/constant/app';
...
function setupTempCache(): { cleanup: () => void } {
  ...
  const prev = process.env[MYCEL_HOME_ENV];
  process.env[MYCEL_HOME_ENV] = home;
  return {
    cleanup: () => {
      if (prev === undefined) delete process.env[MYCEL_HOME_ENV];
      else process.env[MYCEL_HOME_ENV] = prev;
