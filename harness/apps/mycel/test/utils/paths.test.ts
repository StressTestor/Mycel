...
describe('getDataDir', () => {
  it('returns ~/.mycel when MYCEL_HOME is not set', () => {
    expect(getDataDir()).toBe(join(homedir(), '.mycel'));
  });

  it('returns MYCEL_HOME when set', () => {
    process.env['MYCEL_HOME'] = '/tmp/mycel-test-data';
    expect(getDataDir()).toBe('/tmp/mycel-test-data');
  });

  it('returns MYCEL_HOME even if it is a relative path', () => {
    process.env['MYCEL_HOME'] = 'relative/path';
...
describe('getLogDir', () => {
  it('returns <dataDir>/logs', () => {
    expect(getLogDir()).toBe(join(homedir(), '.mycel', 'logs'));
  });

  it('respects MYCEL_HOME', () => {
    process.env['MYCEL_HOME'] = '/z';
...
describe('getBinDir', () => {
  it('returns <dataDir>/bin', () => {
    expect(getBinDir()).toBe(join(homedir(), '.mycel', 'bin'));
  });

  it('respects MYCEL_HOME', () => {
    process.env['MYCEL_HOME'] = '/custom-bin-home';
...
describe('getUpdateStateFile', () => {
  it('returns <dataDir>/updates/latest.json', () => {
    expect(getUpdateStateFile()).toBe(join(homedir(), '.mycel', 'updates', 'latest.json'));
  });

  it('respects MYCEL_HOME', () => {
    process.env['MYCEL_HOME'] = '/updates-home';
...
describe('getUpdateInstallStateFile', () => {
  it('returns <dataDir>/updates/install.json', () => {
    expect(getUpdateInstallStateFile()).toBe(
      join(homedir(), '.mycel', 'updates', 'install.json'),
    );
  });
...
describe('getInputHistoryFile', () => {
  it('returns <dataDir>/user-history/<md5(workDir)>.jsonl', () => {
    ...
    expect(getInputHistoryFile(workDir)).toBe(
      join(homedir(), '.mycel', 'user-history', `${hash}.jsonl`),
    );
  });

  it('respects MYCEL_HOME', () => {
    process.env['MYCEL_HOME'] = '/custom/data';
