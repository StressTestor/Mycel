    ...
    addLifecycleCommands(server, {
      resolveManager: () => ({
        install: async (args) => {
          ...
          return {
            status: 'replaced',
            message: 'Mycel server LaunchAgent replaced at /tmp/kimi.plist (port 9999).',
  ...
  it('start prints URL and diagnostics when launchd did not keep the service running', async () => {
    ...
    addLifecycleCommands(server, {
      resolveManager: () => ({
        ...
        uninstall: async () => ({ ok: true, message: 'unused' }),
        start: async () => ({ ok: true, message: 'Mycel server started (ai.moonshot.kimi-server).' }),
        stop: async () => ({ ok: true, message: 'unused' }),
        restart: async () => ({ ok: true, message: 'unused' }),
        status: async () => ({
          platform: 'darwin',
...
describe('`mycel server run` background start', () => {
  ...
  it('prints a TUI-style ready panel once the daemon is up', async () => {
    ...
    expect(plain.indexOf('Mycel server ready')).toBeLessThan(plain.indexOf('Local:'));
...
describe('ready banner reflects the bind class (M6.3)', () => {
  it('lists Local + Network addresses for a 0.0.0.0 bind (Vite-style)', async () => {
    ...
    expect(raw).toContain('Mycel server ready');
...
describe('mycel web / `server run` (shares `server run` call stack, background default)', () => {
  it('prints the ready banner and opens the browser by default', async () => {
    ...
    expect(stripAnsi(stdout)).toContain('Mycel server ready');
    expect(openUrl).toHaveBeenCalledWith('http://127.0.0.1:58627');
  });
...
describe('`mycel server kill`', () => {
  ...
  it('prints "No running Mycel server." and sends no signal when no live lock exists', async () => {
    ...
    expect(writes.join('')).toContain('No running Mycel server.');
