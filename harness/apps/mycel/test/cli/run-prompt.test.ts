        ...
        if (outputFormat === 'stream-json') {
          ...
          stdout.write(
            `${JSON.stringify({
              ...
              command: 'mycel -r ses_prompt',
              content: 'To resume this session: mycel -r ses_prompt',
            })}\n`,
          );
          return;
        }
        stderr.write(`mycel version ${version}\n`);
        stdout.write('• hello world\n\n');
        stderr.write('To resume this session: mycel -r ses_prompt\n');
      },
    ),
...
describe('runPrompt', () => {
  ...
  it('creates a fresh auto-permission session and streams assistant output to stdout', async () => {
    ...
    expect(stderr.text()).toBe('To resume this session: mycel -r ses_prompt\n');
  ...
  it('formats thinking and assistant output as transcript blocks', async () => {
    ...
    expect(stderr.text()).toBe(
      '• The user wants an exact reply.\n  No tools are needed.\n\nTo resume this session: mycel -r ses_prompt\n',
    );
  ...
  it('writes stream-json output as assistant JSONL with resume meta without transcript bullets', async () => {
    ...
    expect(stdout.text()).toBe(
      [
        ...
        '{"role":"meta","type":"session.resume_hint","session_id":"ses_prompt","command":"mycel -r ses_prompt","content":"To resume this session: mycel -r ses_prompt"}',
        '',
      ].join('\n'),
    );
    expect(stderr.text()).toBe('');
  ...
  it('emits the version first in text mode when the experimental flag is enabled', async () => {
    ...
    expect(stderr.write).toHaveBeenNthCalledWith(1, 'mycel version 1.2.3-test\n');
    expect(stderr.text().startsWith('mycel version 1.2.3-test\n')).toBe(true);
    expect(stdout.text()).toBe('• hello world\n\n');
  });
  ...
  it('does not emit the version when the experimental flag is disabled', async () => {
    ...
    expect(stderr.text()).not.toContain('mycel version');
  });
