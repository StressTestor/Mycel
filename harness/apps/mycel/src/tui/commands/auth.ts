...
import { DEFAULT_OAUTH_PROVIDER_NAME, OAUTH_PROVIDER_DISPLAY_NAME } from '../constant/kimi-tui';
...
async function handleOpenPlatformLogin(
  ...
  const subtitleLines = [
    ...
    `${'saved to'.padEnd(12)}~/.mycel/config.toml`,
  ];
...
export async function handleLogoutCommand(host: SlashCommandHost): Promise<void> {
  ...
  if (hasManagedRemnant) {
    options.push({
      ...
      label: OAUTH_PROVIDER_DISPLAY_NAME,
  ...
  const label = target === DEFAULT_OAUTH_PROVIDER_NAME ? OAUTH_PROVIDER_DISPLAY_NAME : target;
