 ...
 *
 * Default (foreground): the TUI shuts down and the Mycel server takes over this
...
export async function handleWebCommand(host: SlashCommandHost, args: string): Promise<void> {
      ...
      options: [
        {
          ...
          description: background
            ? 'Start the Mycel server (background daemon if needed), open this session in your default browser, and exit the terminal UI.'
            : 'Start the Mycel server in the foreground (this terminal stays attached; Ctrl+C stops it), open this session in your default browser, and exit the terminal UI.',
        },
        {
          value: WEB_CANCEL,
          label: 'Cancel',
  ...
  host.showStatus('Starting Mycel server and opening web UI…');
  ...
  // No server is running: shut the TUI down and let the Mycel server take over
