...
import { LEGACY_HOME_ENV, MYCEL_HOME_ENV } from '#/constant/app';
...
export function registerAcpCommand(parent: Command): void {
  parent
    .command('acp')
    .description('Run Mycel as an Agent Client Protocol (ACP) server over stdio.')
    .option(
      '--login',
    ...
    .action(async (opts: { login?: boolean }) => {
      ...
      // sandboxed test setups (Zed's `agent_servers.*.env.MYCEL_HOME =
      // /tmp/...`). Prefers MYCEL_HOME, falls back to the legacy
      // KIMI_CODE_HOME. Production runs leave the env unset and the field
      // stays empty.
      ...
      try {
        await runAcpServer(harness, {
          agentInfo: { name: 'Mycel CLI', version: getVersion() },
