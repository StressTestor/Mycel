/**
 * `mycel server` parent command. Mounts:
 ...
 *
 * The top-level `mycel web` alias is registered separately via
...
export function registerServerCommand(program: Command): void {
  const server = program
    .command('server')
    .description('Run the local Mycel server (REST + WebSocket + web UI).');

  buildRunCommand(
    server.command('run').description('Start the Mycel server (background daemon; use --foreground to attach).'),
    { defaultOpen: false },
  );
  ...
  // daemon (`mycel web`) over service-ization. The implementation still lives in
