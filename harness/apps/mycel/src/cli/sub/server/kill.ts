...
export function registerKillCommand(server: Command): void {
  server
    .command('kill')
    .description('Stop the running Mycel server (graceful API + forced PID kill).')
    .action(async () => {
      try {
...
export async function handleKillCommand(deps: KillCommandDeps): Promise<void> {
  ...
  if (!lock) {
    deps.stdout.write('No running Mycel server.\n');
    return;
  }
  ...
  if (await waitForExit(pid, TERM_GRACE_MS, deps)) {
    deps.stdout.write(`Mycel server (pid ${String(pid)}) stopped.\n`);
    return;
  }
  ...
  if (await waitForExit(pid, KILL_GRACE_MS, deps)) {
    deps.stdout.write(`Mycel server (pid ${String(pid)}) killed.\n`);
    return;
  }

  throw new Error(
    `Failed to stop Mycel server (pid ${String(pid)}); insufficient permissions?`,
  );
}
