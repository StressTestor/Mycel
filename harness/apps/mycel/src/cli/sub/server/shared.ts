/**
 * Shared helpers for `mycel server …` subcommands.
 *
...
/** Filename (under the Mycel home dir) of the persistent server bearer token. */
...
export interface ServerCliOptions {
  ...
  keepAlive?: boolean;
  /** Internal flag set by the daemon spawner (`mycel web`). */
  daemon?: boolean;
...
export async function ensureServerWebReady(origin: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      `Server at ${origin} does not serve the Mycel web UI${reason}. Stop the existing server and rerun \`mycel server run\`.`,
      { cause: error },
    );
  } finally {
    clearTimeout(timeout);
 ...
 * is the CLI's own home-dir resolution (`getDataDir()`).
 *
