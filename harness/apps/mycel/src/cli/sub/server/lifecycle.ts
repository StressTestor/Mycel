...
export function addLifecycleCommands(parent: Command, deps: LifecycleCommandDeps = DEFAULT_DEPS): void {
  parent
    .command('install')
    .description('Install the Mycel server as an OS-managed service (launchd/systemd/schtasks).')
  ...
  parent
    .command('uninstall')
    .description('Uninstall the Mycel server service.')
    .option('--json', 'Output JSON', false)
  ...
  parent
    .command('start')
    .description('Start the Mycel server service.')
    .option('--json', 'Output JSON', false)
    .action(async (opts: JsonCliOptions) => {
      await runLifecycle(deps, opts.json === true, async (mgr) => {
  ...
  parent
    .command('stop')
    .description('Stop the Mycel server service.')
    .option('--json', 'Output JSON', false)
    .action(async (opts: JsonCliOptions) => {
      await runLifecycle(deps, opts.json === true, async (mgr) => {
  ...
  parent
    .command('restart')
    .description('Restart the Mycel server service.')
    .option('--json', 'Output JSON', false)
    .action(async (opts: JsonCliOptions) => {
      await runLifecycle(deps, opts.json === true, async (mgr) => {
  ...
  parent
    .command('status')
    .description('Show Mycel server service status and connectivity.')
    .option('--json', 'Output JSON', false)
    .action(async (opts: JsonCliOptions) => {
      await runLifecycle(deps, opts.json === true, async (mgr) => {
