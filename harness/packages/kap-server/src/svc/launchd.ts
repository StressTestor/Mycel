...
export function createLaunchdManager(
  ...
  async function install(args: InstallArgs): Promise<InstallResult> {
    ...
    return {
      ...
      message: `Mycel server LaunchAgent ${alreadyInstalled ? 'replaced' : 'installed'} at ${plistPath} (port ${plan.port}).`,
      plistPath,
    };
  }
  ...
  async function start(): Promise<LifecycleResult> {
    ...
    if (!existsSync(plistPath)) {
      return {
        ok: false,
        message: 'LaunchAgent is not installed. Run `mycel server install` first.',
      };
    }
    ...
    return { ok: true, message: `Mycel server started (${KIMI_SERVER_LABEL}).` };
  }
  ...
  async function stop(): Promise<LifecycleResult> {
    ...
    return { ok: true, message: `Mycel server stopped (${KIMI_SERVER_LABEL}).` };
  }
  ...
  async function restart(): Promise<LifecycleResult> {
    ...
    return { ok: true, message: `Mycel server restarted (${KIMI_SERVER_LABEL}).` };
  }
...
function writePlist(plistPath: string, plan: InstallPlan, logPath: string): void {
  const xml = buildLaunchAgentPlist({
    label: KIMI_SERVER_LABEL,
    comment: 'Mycel local server (managed by `mycel server install`).',
