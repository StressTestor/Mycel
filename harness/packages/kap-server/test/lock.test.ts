/**
 * Lock file semantics (ROADMAP P0.12 AC) — ported from v1.
 *
...
describe('startServer — single-instance lock wiring', () => {
  ...
  it('refuses a second server on the same homeDir and releases the lock on close', async () => {
    home = mkdtempSync(join(tmpdir(), 'kimi-server-v2-lock-wiring-'));
    first = await startServer({ host: '127.0.0.1', port: 0, homeDir: home, logLevel: 'silent' });

    // A second instance on the same home must fail fast with ServerLockedError.
    await expect(
      startServer({ host: '127.0.0.1', port: 0, homeDir: home, logLevel: 'silent' }),
    ).rejects.toThrow(ServerLockedError);

    // After the first server closes, the lock is released so a fresh instance
    // on the same home can start (stale-takeover / reuse path).
    await first.close();
    first = undefined;
