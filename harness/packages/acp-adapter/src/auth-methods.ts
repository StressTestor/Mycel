 ...
 *
 * Optional inputs:
 *  - `env`: extra env vars forwarded to the spawned `mycel login`
...
export function buildTerminalAuthMethod(
  ...
  const method: AuthMethod = {
    ...
    // `--login` flag on `mycel acp` pivots into the login flow before
    // touching stdio.
    args: ['--login'],
    env: { ...env },
  };
  if (opts.legacyCommand !== undefined && opts.legacyCommand.length > 0) {
    (method as AuthMethod & { _meta: { 'terminal-auth': unknown } })._meta = {
      'terminal-auth': {
        ...
        // `<command> login` runs the top-level `mycel login` subcommand,
