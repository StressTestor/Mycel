...
describe('OpenAIResponsesChatProvider', () => {
  it('omits max_output_tokens when the upstream does not support it', async () => {
    const provider = new OpenAIResponsesChatProvider({
      model: 'gpt-5.6-sol',
      apiKey: 'test-key',
      maxOutputTokensSupported: false,
    }).withMaxCompletionTokens(1234);
  ...
  it('puts request-scoped bearer auth and provider headers on the wire', async () => {
    let requestHeaders: Record<string, string | string[] | undefined> | undefined;
    const server = createServer((request, response) => {
      requestHeaders = request.headers;
      request.resume();
      response.writeHead(200, { 'content-type': 'application/json' });
      response.end(JSON.stringify(makeResponsesAPIResponse()));
    });
