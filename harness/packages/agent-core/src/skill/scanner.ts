...
// Relative to brandHomeDir, which already IS the brand data dir (~/.mycel or
// $MYCEL_HOME) — no '.mycel' segment here, or it would nest twice.
const USER_BRAND_DIRS = ['skills'] as const;
const USER_GENERIC_DIRS = ['.agents/skills'] as const;
const PROJECT_BRAND_DIRS = ['.mycel/skills'] as const;
...
export interface SkillPathContext {
  ...
  /**
   * Brand data dir — `MYCEL_HOME`, or `<userHomeDir>/.mycel` by default.
   * User brand skills live directly under here as `skills/`, so this path
   * carries no `.mycel` segment of its own (that would double the prefix).
   */
...
export async function resolveSkillRoots(
  ...
  const brandHomeDir = options.paths.brandHomeDir ?? path.join(userHomeDir, '.mycel');
