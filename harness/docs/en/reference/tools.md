...
| `Workflow` | Auto-allow in swarm mode; otherwise requires approval | Launch a phased multi-agent workflow in the background |
...
**`Workflow`** runs a declarative JSON plan as a durable background task. Phases run sequentially, tasks within one phase run in parallel, and a later phase can include an earlier result with <code v-pre>{{result:task_id}}</code>; call arguments use <code v-pre>{{arg:key}}</code>. Pass either an inline `plan` or the name of a saved plan from `<MYCEL_HOME>/workflows/<name>.json`. Interactive sessions expose this experimental tool when `KIMI_CODE_EXPERIMENTAL_DYNAMIC_WORKFLOWS=1`; `/hyphae` combines `xhigh` thinking with swarm-mode authorization. Print mode (`mycel -p`) and ACP expose a reduced surface automatically, while Node SDK hosts opt in with `createKimiHarness({ programmaticWorkflows: true })`. Programmatic workflows allow at most three worker subagents across all phases; the parent Agent is not counted. A `Workflow` call must be the only tool call in its model response, and workflow workers cannot launch nested agents, swarms, or workflows.
...
Background task tools manage tasks started via `Bash`, `Agent`, `Workflow`, or `AskUserQuestion`. When a task reaches a terminal state, its status and saved output path are automatically delivered back to the Agent; use `TaskOutput` to check progress early.
