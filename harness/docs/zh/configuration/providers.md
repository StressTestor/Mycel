...
## Codex ChatGPT 订阅（实验性）

Mycel 可以复用已安装 Codex CLI 的登录状态，同时继续由 Mycel 控制 Agent 循环、工具、审批、hooks 和 gate；Codex 只提供模型响应。

该供应商默认关闭，并依赖未公开的 ChatGPT 后端，因此 Codex 或后端变更后可能失效。需要受支持且稳定的调用路径时，请保留一个使用 API 密钥的供应商。

1. 安装当前版本的 Codex CLI，并运行 `codex login`。
2. 打开生成的 `~/.mycel/config.toml`，取消注释从 `config/mycel.config.toml.template` 复制的 `provider: codex ChatGPT subscription` 配置块。
3. 使用已配置的模型启动 Mycel：
...
适配器通过 `codex app-server` 获取短期请求凭据，不会读取或复制 Codex 的 refresh token。为避免凭据泄露，`storage = "codex"` 只允许用于采用模板固定订阅端点的 `openai_responses` 模型供应商；自定义供应商地址以及内置搜索、抓取服务都会拒绝该凭据来源。
