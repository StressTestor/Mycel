...
| `oauth` | `table` | 否 | OAuth 凭据引用（`storage`、`key` 两个字段）。`storage = "codex"` 为实验功能，仅限[平台与模型](./providers.md#codex-chatgpt-订阅实验性)中固定的 Codex 订阅供应商使用 |
...
`experimental` 存放实验功能 flag 的持久化覆盖。所有字段默认均为 `false`。

| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `micro_compaction` | `boolean` | `false` | 清理较旧的大型工具结果内容，同时保留最近对话 |
| `codex_subscription_auth` | `boolean` | `false` | 允许固定的 `openai_responses` 供应商复用已安装 Codex CLI 的登录状态；详见 [Codex ChatGPT 订阅](./providers.md#codex-chatgpt-订阅实验性) |
...
| `oauth` | `table` | 否 | OAuth 凭据引用，结构同 `providers.*.oauth`；服务凭据会拒绝 `storage = "codex"` |
