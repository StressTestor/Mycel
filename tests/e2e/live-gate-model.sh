...
#
# usage: bash tests/e2e/live-gate-model.sh [existing-auth-home]
#   existing-auth-home: dir containing credentials/ + oauth/ (default ~/.mycel)
...
AUTH_SRC="${1:-}"
if [ -z "$AUTH_SRC" ]; then
  for c in "$HOME/.mycel"; do
    [ -d "$c/credentials" ] && AUTH_SRC="$c" && break
  done
fi
[ -n "$AUTH_SRC" ] || _abort "no auth source found (need credentials/ + oauth/ in ~/.mycel, or pass a dir)"
...
_phase "benign command through live agent"
...
_phase "seeded antibody blocks through live agent"
